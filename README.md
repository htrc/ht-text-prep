...
### Function: `normalize_txt_file_names(directory_path: str, prnt=False)`
    ...
    Given an input path to a single directory holding page text files, this function will normalize 
    ...
    `prnt`: parameter that determines if print messages are returned for each successfully 
    normalized file. By default, this value is `False`.
