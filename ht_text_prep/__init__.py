...
def get_zips(data_dir: str, output_dir: str):
    ...
    # output_dir : str, path to new output folder that will hold the cleaned/processed data. Will return error if
    # folder already exists.

    # delete_zips : bool, default False, provide value True to delete original zipfiles after expansion,
    # False to keep them.
...
def rename_file(file_path):
    ...
    try:
        ...
        print("file names must contain underscores between text containing non-numeric characters and numeric "
              "characters. i.e., 'scout_001.txt'. Insert underscores to rename the following file:", fname)
...
def normalize_txt_file_names(dir_path: str, prnt=False):
    ...
    if dir_path.exists() and dir_path.is_dir():
        ...
        if not txt_files:
            error_message("No txt files found" +
...
def clean_vol(vol_dir_path: str | list, clean_dir_path: str):
    ...
    # Usage ------- Use clean_vol to concatenate the pages and remove headers and footers of every hathitrust volume
    # in a directory or a list of the paths to those volumes. The concatenated, cleaned files are then saved to
    # another directory, clean_dir_path.
